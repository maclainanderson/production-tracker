...
<?php
/**
  *	@desc create/edit/list cut types
*/
	//get user lists
	...
	//fetch data
	if ($conn) {
		$result = sqlsrv_query($conn, "SELECT * FROM Cut_Types");
		if ($result) {
...
<script type="text/javascript">
	...
	*/
	...
	/**
	  *	@desc	fint type by array index
	  *	@param	int i - array index
	  *	@return	none
	*/
	function find(i) {
		current = parseInt(i);
		document.getElementById("cut-input").value = types[current]['TYPE'];
	...
	*/
	function newType() {
		...
		document.getElementById("cut-input").readOnly = false;
	...
	/**
	  *	@desc	prep readOnly attributes to edit a type
	  *	@param	none
	  *	@return	none
	*/
	function edit() {
	...
	*/
	function saveType(s){
		...
		var type = document.getElementById("cut-input").value;
		...
		conn.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				if (conn.responseText.includes("Insert succeeded") || conn.responseText.includes("Data updated")) {
					alert("Changes saved");
					window.location.replace("cuttypes.php");
				} else {
	...
	*/
	function cancel() {
		document.getElementById("cut-input").readOnly = true;
		...
		conn.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				if (conn.responseText.includes("Deletion succeeded")) {
					alert("Cut type deleted");
					window.location.replace("cuttypes.php");
				} else {
					alert("Cut type not deleted. Contact IT Support. " + conn.responseText);
				}
			}
		}
	...
	*/
	...
	/**
	  *	@desc	set onclick function to close modal form
	  *	@param	none
	  *	@return	none
	*/
	function closeForm() {
...
<html>
	<head>
		<title>Cut Types</title>
		<link rel="stylesheet" type="text/css" href="/styles/cuttypes.css">
	...
	<body onload="insertFirst()">
		<div class="outer">
			<div class="inner">
				<div class="content">
					<span id="cut-span">Cut Type<input id="cut-input" type="text" readonly></span><br>
