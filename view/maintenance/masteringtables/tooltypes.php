...
<?php
/**
  *	@desc list of mastering tool types
*/
	//get user lists
	...
	//fetch data
	if ($conn) {
		$result = sqlsrv_query($conn, "SELECT * FROM Tool_Types");
		if ($result) {
...
<script type="text/javascript">
	...
	*/
	function find(i) {
		current = parseInt(i);
		document.getElementById("tool-input").value = types[current]['TOOLTYPE'];
	...
	/**
	  *	@desc	prep readOnly attributes and value for a new type
	  *	@param	none
	  *	@return	none
	*/
	function newType() {
	...
	/**
	  *	@desc	prep readOnly attributes for editing a type
	  *	@param	none
	  *	@return	none
	*/
	function edit() {
	...
	*/
	function saveType(s){
		...
		conn.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				if (conn.responseText.includes("Insert succeeded") || conn.responseText.includes("Data updated")) {
					alert("Changes saved");
					window.location.replace("tooltypes.php");
				} else {
		...
		conn.open("GET","/db_query/sql2.php?action="+action+"&table="+table+"&TOOLTYPE="+type+"&STATUS="+state+"&DATE="+date+"&OPERATOR=<?php echo $_SESSION['initials']; ?>&id="+id, true);
		conn.send();
	}
	...
	*/
	function search() {
		...
		types.forEach((item, index, array) => {
			html += '<tr id="'+index+'" onclick="selectRow(this)"><td class="col1">'+item['TOOLTYPE']+'</td><td class="col2">'+item['STATUS']+'</td><td class="col3">'+item['DATE']+'</td><td class="col4">'+item['OPERATOR']+'</td></tr>';
		});
...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="/styles/tooltypes.css">
	...
	<body onload="insertFirst()">
		<div class="outer">
			<div class="inner">
				<div class="content">
					<span id="tool-span">Tool Type<input id="tool-input" type="text" readonly></span><br>
