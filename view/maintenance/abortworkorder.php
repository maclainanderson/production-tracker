...
<?php
/**
  *	@desc manage reasons to abort a job
*/
	//get user lists
	...
	//fetch data
	if ($conn) {
		$result = sqlsrv_query($conn, "SELECT * FROM Abort_Work_Order");
		if ($result) {
...
<script type="text/javascript">
	...
	*/
	function saveReason(s) {
		...
		conn.open("GET","/db_query/sql2.php?action="+type+"&table="+table+"&REASON="+reason+"&STATUS="+state+"&DATE="+date+"&OPERATOR=<?php echo $_SESSION['initials']; ?>&condition=id&value="+id,true)
		conn.send();
	}
					...
					if (this.responseText.includes("Data updated")) {
						...
						if (attempts >= conn.length) {
							if (successes >= conn.length) {
								alert("Changes saved");
								window.location.replace("abortworkorder.php");
							} else {
...
<html>
	<head>
		<title>Abort Work Order</title>
		<link rel="stylesheet" type="text/css" href="/styles/abortworkorder.css">
